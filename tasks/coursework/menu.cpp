...
void loop_main(LOOP_ARGS_DEFINITION) {
    ...
    do {
        ...
        case '3':
            ...
            if (exec_node) info_tasks_of_executor(os, exec_node->value);
            break;
        case '4':
...
void loop_date_select(LOOP_ARGS_DEFINITION) {
    ...
    do {
        ...
        is >> begin;
        if (begin.is_empty()) os << "Введите валидную дату\n";
    } while (begin.is_empty());
    do {
        os << "Конец периода в формате (YYYY MM DD).\nДля выбора одного для, повторите его здесь: ";
        is >> end;
        if (end.is_empty()) os << "Введите валидную дату\n";
    } while (end.is_empty());
...
void info_tasks_date_by_executors(
    std::ostream& os,
...
ExecutorN* loop_executor_select(LOOP_ARGS_DEFINITION) {
    ExecutorN* p = nullptr;
    do {
        os << "Введите табельный номер исполнителя (0 для выхода из выбора исполнителя): ";
        unsigned number;
        is >> number;
        p = executors.first;
        if (number == 0) return nullptr;
        while (p and p->value->service_number != number) p = p->next;
        if (!p) os << "Исполнитель не найден, попробуйте ещё раз\n";
    } while (!p);
    return p;
}
...
void loop_executors_group_select(LOOP_ARGS_DEFINITION) {
    ...
    os << '\n';
    info_group_tasks(os, virtual_executors, e_count, virtual_tasks_selected, t_count);
    virtual_executors.destroy();
    virtual_tasks_selected.destroy();
}
void info_group_tasks(
    std::ostream& os,
    list<Executor> executors, unsigned e,
    list<Task> tasks, unsigned t
) {
    ...
    while (ep) {
        os << '\t'; ep->value->name.print(os);
        os << '\n';
        file::out() << '\t'; ep->value->name.print(file::out());
        file::out() << '\n';
        ep = ep->next;
    }
    os << '\n';
    ...
    while (tp) {
        tp->value->name.print(os); os << '\n';
        tp->value->name.print(file::out()); file::out() << '\n';
        if (!executors.is_empty()) {
            os << "\tИсполнители:\n";
